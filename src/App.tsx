...
import { isIE, msalApp, GRAPH_REQUESTS, requiresInteraction } from './auth/authUtils';
import { logout, loadAccount } from './redux/actions/ApplicationActions';
...
function mapStateToProps(state: StoreState) {
	return {
		...
		msalIdToken: state.application.msalIdToken
	}
}
...
class App extends React.Component<AppProps, AppState> {
	...
	private login() {
		...
		msalApp.loginPopup(GRAPH_REQUESTS.LOGIN).then(res => {
			if(res) {
				this.props.loadAccount(res.account);
				this.acquireToken();
			} else {
				this.setState({
					error: true,
					errorDetails: "Unable to access system at this time"
				});
			}
		})
		.catch(error => {
	...
	private acquireToken() {
		store.dispatch({ type: SET_NEW_ACCESS_TOKEN, payload: "testToken"});
		store.dispatch({ type: SET_NEW_MSAL_TOKEN, payload: "testToken"});

		/*msalApp.acquireTokenSilent(GRAPH_REQUESTS.LOGIN).then(res => {
			// store.dispatch({ type: SET_NEW_ACCESS_TOKEN, payload: res.accessToken});
			// store.dispatch({ type: SET_NEW_MSAL_TOKEN, payload: res.idToken.rawIdToken});
		//@ts-ignore
		}).catch(error => {
			...
			if (requiresInteraction(error.errorCode)) {
				// console.log("Requires interaction");
				//return redirect
	...
	public render() {
		...
		return(
			<div className="App">
				<Slide direction="right" in={!loggedIn} mountOnEnter unmountOnExit>
					<div style={{ position: 'relative', zIndex: 10000, margin: 0, padding: 0, fontFamily: 'sans-serif', minHeight: '100vh', display: 'flex', width: '100%', backgroundColor: '#282c34'}}>
						...
						<div style={{
							...
							<div style={{fontSize: 22, fontWeight: 315, marginBottom: 10}}>Demo App</div>
							<div style={{color: 'white', fontSize: '9pt'}}>
								<p style={{margin: '0px'}}>{'Version ' + PackageJSON.version}</p>
							</div>
							<div id="microsoftLogin" style={{marginTop: 25}} onClick={this.login}>
								<span style={{marginTop: 3, marginRight: 10}}>
									Login using 
								</span>
								<span style={{width: 150}}>
									<svg data-slug-id="msazure-logo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 506 72">
										<path fillRule="evenodd" clipRule="evenodd" d="M20.244 8.014L13.857 26.209H26.811L20.455 8.014H20.244ZM24.702 0L40.97 43.201H32.716L28.89 32.476H11.869L8.194 43.201H0L16.267 0H24.702Z" transform="translate(350.999 14.3985)" fill="white"></path>
										<path fillRule="evenodd" clipRule="evenodd" d="M1.145 0H25.878V2.923L9.64 25.186H25.968V30.97H0V27.505L15.997 5.784H1.145V0Z" transform="translate(394.539 26.6293)" fill="white"></path>
										<path fillRule="evenodd" clipRule="evenodd" d="M27.806 0V30.97H20.455V26.902H20.335C19.371 28.309 18.08 29.408 16.464 30.201C14.847 30.994 13.035 31.392 11.027 31.392C7.431 31.392 4.695 30.372 2.817 28.333C0.939 26.295 0 23.127 0 18.829V0H7.381V17.955C7.381 20.566 7.898 22.529 8.933 23.845C9.967 25.16 11.528 25.818 13.618 25.818C15.686 25.818 17.342 25.09 18.588 23.634C19.833 22.178 20.455 20.274 20.455 17.925V0H27.806Z" transform="translate(424.786 26.6293)" fill="white"></path>
...
export default connect(mapStateToProps, { logout, loadAccount })(App);
