...
import { InputBase, TextField, MenuItem, Button, LinearProgress, Card, CardContent } from '@material-ui/core';
...
import { getPurchaseAgreements, getPurchaseAgreementById, getAgreementsByLeadId, getAgreementsByBuyerName, getAgreementsByDeliveryState, getAgreementsByEmail } from '../../services/PurchaseAgreementService';
...
class AllAgreements extends React.Component<AllAgreementProps, AllAgreementsState> {
	constructor(props: AllAgreementProps) {
		super(props);
		this.state = {
			page: 0,
			size: 20,
	...
	public loadAgreements(page: number, size: number, sort: string, searchState: string) {
		const { searchKey } = this.state;

		this.setState({ agreements: [], loading: true });
		switch(searchState) {
			case NO_SEARCH: {
				getPurchaseAgreements(page, size, sort).then(res => {
			...
			case ID_SEARCH: {
				getPurchaseAgreementById(searchKey).then(res => {
					this.setState({ loading: false });
					if(validateHTMLResponse(res)) {
						this.setState({
							agreements: res.data._embedded.purchaseagreements,
			...
			case LEAD_ID_SEARCH: {
				getAgreementsByLeadId(searchKey).then(res => {
					this.setState({ loading: false });
			...
			case BUYER_NAME_SEARCH: {
				getAgreementsByBuyerName(searchKey).then(res => {
					this.setState({ loading: false });
			...
			case DELIVERY_STATE_SEARCH: {
				getAgreementsByDeliveryState(searchKey).then(res => {
					this.setState({ loading: false });
			...
			case EMAIL_SEARCH: {
				getAgreementsByEmail(searchKey).then(res => {
					this.setState({ loading: false });
	...
	public renderRowCells(agreement: PurchaseAgreement) {
		let cells: any[] = [];
		PURCHASE_AGREEMENT_DISPLAY.forEach(header=> {
			if(header.columnDef === 'modificationTime') {
				...
				let leadSalesOffice: SalesOffice = this.state.salesOffices.find(office => office.id === agreement.locationId) || new SalesOffice();
				cells.push(
...
export default connect(null, { updatePathWithObject})(AllAgreements);