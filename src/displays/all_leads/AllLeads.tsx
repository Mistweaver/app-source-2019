...
import { InputBase, Button, LinearProgress, Dialog, Slide } from '@material-ui/core';
...
import { getLeads, getLeadsById, getLeadsByFirstName, getLeadsByLastName } from '../../services/LeadServices';
...
class AllLeads extends React.Component<MyLeadProps, AllLeadsState> {
	...
	public loadLeads(page: number, size: number, sort: string, searchState: string) {
		const { searchKey } = this.state;
		this.setState({ loading: true });
		switch(searchState) {
			case NO_SEARCH: {
				getLeads(page, size, sort).then(res => {
					if(validateHTMLResponse(res)) {
						this.setState({
			...
			case FIRST_NAME_SEARCH: {
				getLeadsByFirstName(searchKey).then(res => {
					if(validateHTMLResponse(res)) {
						this.setState({
							leads: res.data._embedded.leads,
			...
			case LAST_NAME_SEARCH: {
				getLeadsByLastName(searchKey).then(res => {
					if(validateHTMLResponse(res)) {
						this.setState({
							leads: res.data._embedded.leads,
							totalObjects: res.data._embedded.leads.length,
	...
	public renderRowCells(lead: Lead) {
		let cells: any[] = [];
...
export default connect(null, { updatePathWithObject})(AllLeads);